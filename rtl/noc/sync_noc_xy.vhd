...
architecture ring of sync_noc_xy is
...
--      data_s_out    : out std_logic_vector(width-1 downto 0);
  ...
  signal data_void_in_i  : std_logic_vector(2 downto 0);
  signal stop_in_i       : std_logic_vector(2 downto 0);
  signal data_void_out_i : std_logic_vector(2 downto 0);
  signal stop_out_i      : std_logic_vector(2 downto 0);

  begin

  data_void_in_i            <= sync_data_void_in & data_void_in(1 downto 0);
  stop_in_i                 <= sync_stop_in & stop_in(1 downto 0);
  sync_data_void_out        <= data_void_out_i(2);
  data_void_out(1 downto 0) <= data_void_out_i(1 downto 0);
  sync_stop_out             <= stop_out_i(2);
  stop_out(1 downto 0)      <= stop_out_i(1 downto 0);
...
----------------------------------------------------------------------------------------------
    ...
    -- Monitor signals
    mon_noc.clk          <= clk;
    mon_noc.tile_inject  <= not data_void_in(2);

    mon_noc.queue_full(2) <= data_void_out_i(2) nand data_void_in_i(2);
...
-- each channel will have a sync tx and rx pair with prefix FWD and REV respectively
  ...
  no_synchronizers: if HAS_SYNC = 0 generate
    ...
    sync_data_void_in <= data_void_in(2);
    sync_stop_in <= stop_in(2);
    output_port <= sync_output_port;
    data_void_out(2) <= sync_data_void_out;
    stop_out(2) <= sync_stop_out;

  end generate no_synchronizers;

  inferred_async_fifos_gen: if HAS_SYNC /= 0 generate
    begin
      fwd_we_i    <= not data_void_in(2);
      stop_out(2) <= fwd_wr_full_o;-- when data_void_in = '0' else '0';
      ...
      rev_rd_i         <= not stop_in(2);
      data_void_out(2) <= rev_rd_empty_o;-- when stop_in = '0' else '1';
